...
const StyleSelector = ({
  ...
  const styleDescriptions: Record<ThumbnailStyle, string> = {
    ...
    Minimalist: "Clean, Simple, lots of white paper",
      ...
      {isOpen && (
        <div className="absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg">
            {thumbnailStyles.map((style)=>(
                <button key={style} type="button" onClick={()=>{onChange(style); setIsOpen(false);}} className="flex w-full items-start gap-3 px-4 py-3 text-left transitio hover:bg-black/30">
